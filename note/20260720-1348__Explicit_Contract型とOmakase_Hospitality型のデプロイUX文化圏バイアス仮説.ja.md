...
### Q AtlantisはすでにWorld Positionを告白している

Quantaril Cloud Q Atlantisは、中立無思想だから何でも入る空箱も、権威科学的な中立思想も最初から名乗って
いない。Q Atlantis自身のREADME、AGENTS、哲学棚、世界観棚では、神話・スピリチュアルを大切にすること、
縄文・古神道から派生する「縄文2.0」的な系譜、古神道のアニミズム的世界観とtechnologyを結ぶ
Techno-Animism、Gaming Cosmologyを扱うことを、公開されたWorld Positionとして先に告白している。
思想濃度を薄めて平均的な中立へ見せることは、この自己告白の改変になる。

同時にQ Atlantisは、異なる信仰とworld flavorが自分の言葉で参加できる土俵を広く開く、非独占の
「全方位信仰」を取る。ここでの全方位は、相互に矛盾する命題をすべて同時に真と認定することでも、一つの
Worldへ平均化して混ぜることでもない。思想を持たないから受け入れるのではなく、八百万の価値観を持つからこそ、
他者が別の告白を記述し、選び、必要なら別Worldとして分岐できることを守る。

したがってQ Atlantisの「おもてなし」defaultを説明するとき、その出所は「日本人一般なら察するはず」という
無告白の文化圏推測ではない。Q Atlantisが選び、公開し、責任を引き受けたWorld Config／philosophyである。
この告白は、spiritualityを隠したまま中立defaultへ混入させることを避け、利用者と接続先が採用・拒否・変換を
判断できるようにする。

このPositionでは、「科学的中立」を無色透明な零点とは扱わない。権威によって正当化された科学観、平均値、
globalな中央値を普遍defaultへ置くことも、一つの信念、制度、World Positionになり得ると観測する。ただし、
これは科学的方法、検証可能性、再現性、工学的測定を否定する射程ではない。それらを重要な観測技法として
使いながら、その技法が神話、信仰、spirituality、個別Worldの意味を越権して裁定することを拒む。

ただし、このPositionが持つ射程はQ Atlantis Worldおよび明示的に採用したprofileまでである。古神道、
Animism、Techno-Animism、Gaming CosmologyをAtlantis全体の普遍真理、他宗派・他文化圏・他Worldのdefaultへ
昇格させない。他のWorld builderは、自身の信仰、価値、気遣い、拒否境界を自身で告白し、実装する。

Qが八百万の価値観を掲げることは、他者の哲学、信仰、world flavorをQへ同化させる根拠ではなく、他者が自由に
記述し、選ぶ自由を否定しない根拠になる。SphereOS Atlantis側は「お前の信仰告白は、お前がSDKを使って
modelingしろ。それはOSが代筆するものではなく、お前の哲学・信仰である」という非独占の立場を取る。

ここでOSが放棄するのは信仰内容の代理解釈であり、pluralityを支える責務ではない。告白を置く器、射程表示、
World Config、SDK、接続互換の検査、Portal／adapter／退出経路はOS側の設計課題として残る。

異なる告白を持つWorld同士を接続するときは、Hospitality policyとWorld Configの互換を比較する。一致しない
場合は陸続きにせず、Portal、adapter、別profileまたは接続拒否へ分ける。これはQのPositionを消すためでも、
相手へ押しつけるためでもなく、双方のWorld Positionを保ったままBridgeの条件を明示するためである。
...
- Quantaril Cloud Q Atlantis `README.md`、`AGENTS.md`、`docs/philosophy/techno-animism.md`、
  `docs/philosophy/soul-sovereignty.md`、`docs/philosophy/civilization/jomon-vs-yayoi.md`、
  `docs/worlds/index.md`にあるWorld Positionの自己告白
