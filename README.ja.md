...
ここでいうOSSパイレーツは、権利侵害の推奨ではない。放棄された設計をsalvageし、sourceとcommitを読み、
合法にforkし、失敗receiptを持ち帰る独立探索者である。既定は非排他的な贈与コモンズ。
作者、先行思想、神々、code、詩、生活様式へのrespectは、誰か一人へ神やアイデアを独占させるtokenではなく、
Gitのstarのように増やせるlineageとして残す。商用・社内・閉鎖Worldも作れるが、閉じた魔王城の城壁を
core、既存commons、無関係なWorldへ伸ばさない。詳細は
[贈与コモンズ・系譜・局所World拡張契約](docs/operations/gift-commons-lineage-and-local-world-extension.ja.md)を参照。
...
- [assets](assets) — ZeroRoomLab固有のRole／Flavor／Presentation。指定された時だけ読み、local-only非公開物を欠損扱いしない
...
- author、source、revision、関係次元をlineageとして残す。ただし系譜を許可証、本人性、権威、独占rankへ変換しない
- commercial／private Worldは局所scopeで閉じ、core、既存commons、別Worldのfork権を囲い込まない
