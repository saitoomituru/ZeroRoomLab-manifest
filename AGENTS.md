...
[ ] 14. COMMERCIAL KPI SUBSTITUTION CHECK
        Am I treating market share, revenue, sponsor-loss avoidance, enterprise adoption,
        staff count, or procurement convention as proof that a design is universally correct?
        → If yes: STOP. Those metrics describe one operating context, not a truth ruler.
          Preserve the target product's declared purpose and user KPI. When an upstream
          default conflicts with them, keep upstream compatibility where useful and absorb
          the difference with an adapter, profile, or local implementation. Do not overwrite
          gift-economy, solo-performer, minority, or experimental defaults merely because
          commercial actors are more numerous or better funded.
          Technical respect does not require philosophical neutrality. Keep unverified
          claims about an implementation bounded, while allowing the project to explicitly
...
component間の優先順位決定へ入る前に、SphereOS Atlantisが実装正本として配布する現行MAGI bundleと
Context定規・因果・OAE横断監査規約を使い、監査者自身の持ち込みを検査する。

MAGIを使う必要がある場合は、`スフィア.code-workspace`からSphereOS Atlantisを解決し、
Atlantisの`AGENTS.md`、`magi/*/bundle.json`、採用版source map、三Position Skillを明示的に読む。
folder名の文字列比較だけで最新版を推定せず、bundle内の座標、lineage、実装状態を確認する。
repositoryの暗黙scan、常駐daemon、asset auto-mountをこの指示から起動しない。

ZeroRoomLab固有の規約、Role、Flavor、PresentationはManifest側から明示includeする。
...
- 市場規模、売上、スポンサー損失、企業採用数、調達慣行を、個別productの目的関数より上位の正しさへ昇格すること
...
`foldlog/`へ事実・解釈・修正・unknown・Semantic Stopを分離した監査記録を残し、
...
"0.25.1 is ordinary major.minor.patch SemVer" — INCORRECT.
  ...
  `sphere-context-os-coordinate/2`では`UserContextSolution.SDKSeries.ProtocolKernelGeneration`をTarget Contractとする。
  `/2`の先頭はUserからの抽象距離で並べたOS bundle／Runner／GUI・Presentationであり、0=Developer、
  1=Purpose、2=Shared Reality、3=World Law、4=Meta-Worldという主Context責務を表す。
  中段はSDK、右端はProtocol／Kernel世代である。
