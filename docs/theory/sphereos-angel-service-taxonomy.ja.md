...
```text
POSIX / Linux technical layer : daemon / service / systemd unit
SphereOS context runtime      : Angel / ArchiAngel
```

ここでいう意味次元は技術Layerの上位番号ではなく、技術依存軸`L`と直交するContext Dimension `D`である。`L`／`D`、Registry、Access Map、Transformer、OAEの共通定義は[Sphere Context Dimension OSアーキテクチャ](sphere-context-dimension-os.ja.md)を参照する。
...
`ArchiAngel`は、個別処理だけでなく、SphereOS全体の構成・routingへ関与するAngel roleである。ここでの`architectural`はContext Dimensionの本数を表す`D`ではなく、system-wideな設計責務を表す。
...
したがって、`Angel`はdaemonの改名ではなく、下層OSと別責務を持つContext runtimeのservice classである。
