...
状態: `[CANONICAL]` `[LEGACY DISPLAY 0.2.1]` `[SEMANTIC KERNEL 0.200.1]` `[Layer A/B/C bridge]`
制定日: 2026-07-18  
対象: MAGI 0.2.0 Skill bundle、OAE時間整合性、過去資料監査、仮想復元分岐

## 1. 発行理由

Atlantis-MAGISDK `0.2.1`はlegacy distribution表示であり、三層版数座標では`0.200.1`へ写像する。
0.2.0 Skill bundleが過去資料を監査するとき、Source Event／Evidenceと
同時点OAEを区別できず、存在しなかった過去OAEを現在の推論で補完できてしまう脆性を修正する。

0.1.0の三Position、非神格、非多数決、zero-trust、Position-talk監査を保持する。0.2.0で実装された
Skill workflowも保持し、時間整合性契約と機械validatorを追加する。legacy releaseではcorrective patchと
呼ばれたが、意味・因果Kernel上は同一世界線への遡及OAEを許し得るKernel 0から、それを拒否する
Kernel 1への因果法則変更である。0.1.0正本と0.2.0の
実装記録は上書きしない。
...
本修正はlegacy release運用では`0.2.0 -> 0.2.1`のcorrective patchとして発行された。このSource Eventを
消さず、現在の三層版数座標では次へ非破壊mappingする。
...
右端1は軽微修正番号ではない。過去OAEの同一世界線内への遡及配置を拒否し、現在の解釈を新しいOAEへ
appendし、反実仮想を別World／別Instance Ghostへ隔離する因果Kernel世代である。`0.200.0`と
`0.200.1`を直接陸続きにせず、旧Kernel artifactは因果Gateで隔離・投影する。

SphereOS Atlantis Prompt Engineering Editionは、0.2.1 bundle、validator、負例test、doctor検査を
組み込んだcommitを根拠としてlegacy alias `0.2.1-alpha.1`を申請できる。release channel、candidate、
package revisionは三層座標と別fieldで管理する。
