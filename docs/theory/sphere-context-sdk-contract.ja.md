...
### 3.1 PLIはLLM driverではない

`Prompt Line Interface`（PLI）はS4のinteraction surfaceであり、特定のLarge Language Model、provider、
Chat UI、Assistant人格を意味しない。目的、問い、制約、Context、希望する結果を受け、Provenance、
Registry、Fold、unknownを保持したまま、型付きworkflowまたは下位surfaceへ接続する。

`Large Language Model Interface`（LLMI）を設ける場合は、model／provider／inference driverを接続する
S0／S1側のExecution Envelopeまたはadapter境界として扱う。PLIからLLMIの存在、利用成功、model能力、
課金、外部network、権限を推定しない。同じPLIをsmall model、複数model、rule engine、人間review、
将来runtimeへ接続できる余地を残す。

PLIは主にD軸上の高度抽象、nD Fold、World、意味、目的、未決定事項の探索に向き、Command Line
Interface（CLI）は主にL軸上の技術依存、既知command、対象、型、権限、停止条件の高強度拘束に向く。
これは排他的な実装制限ではなく、Surface選択時の適性である。
