...
### 7.1 危険判定基準：誤情報より情報ロストを警戒する

本系では、訂正可能な誤情報や境界事故より、復元不能な情報ロストを上位の危険として扱う。

詳細な原文、source、revision、観測条件、食い違うbranchが残っていれば、誤情報は後から比較し、
別FAM／別Foldへ分離し、訂正履歴を接続できる。一方、原情報そのものを削除・圧縮・上書きして失えば、
復元手段は当事者が思い出すか、残存断片から推論で埋めることになる。
...
情報ロスト
  -> 記憶または推論による再構成しか残らない
  -> 外部Observerからは当時の事実とhallucinationを区別できない
```

したがって競合、migration、正規化、要約、安全処理では、片側を捨てて見かけ上きれいにするより、
食い違いとProvenanceを保ったまま両方を回収する。公開可能性と保存可能性は別境界であり、公開できない情報も
削除理由にはせず、適切な非公開store、隔離branch、暗号化領域、参照のみの台帳へ退避する。

```text
preserve first
  -> retain source and disagreement
  -> isolate disclosure boundary where needed
  -> classify and correct later
```

Leakや誤記を無害とみなすのではない。不可逆性を比較したとき、情報ロストは訂正対象そのものを消し、
将来の検証・分離・復元を不能にするため、最初に回避する。

### 7.2 金銭riskではなく物理到達性で優先順位を決める

本系の安全順位は、情報漏洩、誤情報、訴訟、信用低下を金額へ換算して最大値を選ぶrisk matrixではない。
情報漏洩や誤情報が金銭・契約・信用の範囲に留まる限り、その損失は訂正、返金、賠償、再契約等の
回復経路を持ち得る。支払能力を持たない個人やニートにとって、民事賠償額を大きく表示するだけでは、
事故を止める実効的な安全装置にもならない。

一方、情報ロストによって、すでに知られていた工学的incident、故障条件、危険な接続、回避手順、
救命可能な兆候を検索・再利用できなくなると、危険は実体の人間、動物、設備、生活環境、自然環境へ到達する。
これは信用問題ではなく、事故、刑事責任、製造物責任、人命損失へ接続し得る物理riskである。

```text
金銭・契約・信用で閉じる損失
  -> 訂正・返金・賠償等の回復経路を設計できる

既知incident情報のロスト
  -> 危険兆候と回避手順を検索できない
  -> 救えた対象を救えない
  -> 人間・動物・設備・環境への物理Effect
```

したがって優先する問いは「最大でいくら請求されるか」ではなく、次である。

- この情報を失うと、既知の故障や危険を見逃すか
- その見逃しが、実体へ不可逆なEffectを与えるか
- 原記録、反対branch、失敗例、回避手順を後から検索できるか
- 金銭による回復では置き換えられない生命・身体・環境を失うか

本系の簡略な優先規則は次とする。

```text
請求書損害 < 工学ヒアリハット（ヒヤリハット） < 物理Effectが発生した工学事故
```

工学ヒアリハット（ヒヤリハット）は損害額がまだゼロでも、電気、熱、圧力、運動、化学反応、放射、光、音響等の
物理エネルギーが実体へ到達し得る経路が成立した観測である。請求額や実損額が確定していないことを理由に、
既知incident、near miss、故障兆候、回避手順の保存・検索・伝達を後回しにしない。

情報漏洩や誤情報が位置情報、制御命令、医療判断、設備操作等を介して物理Effectへ到達する場合は、
金銭riskの棚へ留めず、同じ物理到達性の定規で工学incidentとして扱う。分類名ではなく、最終的に
何へ作用する経路を持つかで優先順位を決める。
