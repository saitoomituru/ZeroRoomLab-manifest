...
### 3.1 Embedding実行配置は中立、識別metadataはIBDでbindする
...
生成engineはapplication library、native service、container service、Neo4j plugin、外部providerのどこへ置いてもよい。IBD StoreのMeta Catalogは、各Database／Storage Bindingがどの生成pipeline、metric、calibration profileを使ったかをstable ref、revision、fingerprintとしてbindする。credential本文やmodel weight本体をCatalogへ保存するという意味ではない。

`Embedding Profile`等の最終schema名、本番adapter、model採用は未確定である。一方、実行配置が違っても、比較・再生成・鮮度検査に必要なidentity metadataを失わない責務はIBD側の契約へ置く。
...
同一IBD Database内では、そのDatabaseのembedding profileとmetricへ揃えたvectorを検索する。異なるDatabase／storeではQuery FAMのtext／構造を各engineへ渡してlocal retrievalを行い、raw vectorとraw scoreを直接比較しない。Mapping FAM、rank、calibration profile、uncertaintyを用いてComposite Resolverが非破壊結合する。
...
SsCは異種vectorを一つの座標空間へ変換する機能ではない。各store内のlocal scoreを校正profile経由でmeta SINへ射影し、検索射程をstore固有thresholdへ逆射影する研究module候補である。`sin(θ)=sin(π-θ)`のsector曖昧性と、`atan`が分布校正そのものではない境界を未解決事項として残す。
...
- Sphere共通L／D／OAE: [Sphere Context Dimension OSアーキテクチャ](sphere-context-dimension-os.ja.md)
- SDK共通契約: [Sphere Context SDK共通契約](sphere-context-sdk-contract.ja.md)
- IBD実装プロファイル: [Context Dimension OSにおけるIBDとIBDSDK](https://github.com/saitoomituru/IBD/blob/main/docs/architecture/context-dimension-os-and-ibdsdk.ja.md)
- IBDSDK module契約: [IBDSDK module契約](https://github.com/saitoomituru/IBD/blob/main/docs/specification/ibd-sdk-module-contracts.ja.md)
