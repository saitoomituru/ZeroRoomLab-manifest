...
## 6. FAM三系統：現状ステータス（2026-07-02時点）

FAMは単一の実装ではない。接続先バス（AQC / MCP / Sphere-aae）ごとに技術的血統が異なる**3系統**が並行して存在し、それぞれ検証段階が異なる。**3系統を混同して語らない**——特に、サ終済みの実績を現在進行中のトイモデルの完成度であるかのように語らないこと。

| 系統 | 基盤 | 状態 | 補足 |
|---|---|---|---|
| **FAMoverAQC** | AQC（[aqc.ja.md](aqc.ja.md)） | 🔶 サ終済み（旧世代） | NPO後援によるデプロイとして稼働実績・デプロイ完遂まで到達。市場のレッドオーシャン化・IaaS側事情という外部要因によりサービス終了（X上で告知済み）。現在は利用不可。現行プロジェクトへの参照は規約抵触リスクを都度索敵してから判断する |
| **FAMoverMCP** | MCP | 🔶 トイモデル・未検証段階 | AQC版のサ終後、クラウド/ネットバス依存をやめてローカル/エッジ環境で仕様・コード・設計思想を拾い直す「サルベージ」。現在進行形。AQC版の実績をそのまま流用して語らない |
| **FAMoverSphere-aae** | Sphere-aae（[Sphere-aae リポジトリ](https://github.com/saitoomituru/Sphere-aae)） | 🔶 設計中・HPC調達待ちで停止 | メタAGI生成SDKレベルからエッジ用に再設計している別系統。FAMoverMCPとは技術的血統が異なる（混同しない）。工数が重い理由は抽象度の高いレベルから作り直しているという技術的理由。停止理由はHPC/メモリ環境の物理的制約であり、実装力・設計の欠如ではない |

**具体例（DVE800）**: [dve800.ja.md](../projects/dve800.ja.md) のModule 3（`aqc_local`）は、FAMoverAQCのフルレベル実装（過去に一度動作した実績があるが現状は再現不可・サルベージ対象）ではなく、**FAMoverMCPのトイモデル検証**として実装されている。「FAMが動く」という記述を見た場合、必ずどの系統のどの検証段階かを確認すること。

---

## 関連ドキュメント

- [IBD FAMネイティブResolverとバインダー中立性](ibd-fam-native-binder.ja.md) — FAM探索技を隔離保存し、非破壊でComposite FAMへ結合するIBD Query Profile
