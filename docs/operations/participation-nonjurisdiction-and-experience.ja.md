...
personaは習熟度または実装意図ではない。tutorial開始時の`proficiency`は`unknown`、`intent`は
`look-around`を既定とし、利用者が実装を明示的に選ぶまでbranch作成やcode変更へ進まない。現在能力、
未実装境界、非code入口、次の選択肢は
[Help／Capability Discovery共通契約](help-and-capability-discovery.ja.md)に従って提示する。
...
- personaからmaintainer相当の習熟度や実装意図を推定せず、Help開始だけでmutationしない
