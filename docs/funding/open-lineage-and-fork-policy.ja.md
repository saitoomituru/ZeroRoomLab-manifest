...
- 使用モデルの正確な名称、版、重み、量子化、ハッシュ、入手経路
- モデルを再配布できない場合、その制約と完全に同一の取得・検証手順
...
「モデル非依存」は、何を使っても正統になるという意味ではありません。上流は特定モデルを神体としてバンドルしませんが、派生バンドルが正統性を主張するなら、実際に使ったモデルと内部実装を含めて開示します。
